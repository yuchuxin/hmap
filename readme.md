# HMap 文档

> `hmap` 是一个基于 Go 语言实现的线程安全、高并发 Map。通过将数据分散存储到多个分片（Shards）中，有效减少了高并发场景下的锁竞争，提升读写性能。

---

## 📖 简介

`hmap` 是一个 Go 语言实现的泛型并发安全 Map，采用分片锁（Sharding）技术降低锁竞争，适用于高并发读写场景。

**特性：**
*   **✅线程安全**：支持多 Goroutine 并发读写。
*   **✅高性能**：采用分片锁（Sharding）机制，显著降低锁粒度。
*   **✅泛型支持**：支持存储任意类型的值（`[V any]`），Key 固定为 `string`。
*   **✅零拷贝哈希**：使用 `sync.Pool` 复用 Hash 对象，减少 GC 压力。
*   **✅死锁预防**：`Range` 操作采用快照机制，支持在遍历回调中安全地操作 Map。
*   **✅位运算优化**：自动调整分片数量为 2 的幂次，利用位运算加速定位。

---

## ⚠️ 环境要求

*   **Go 1.21+** (依赖标准库 `maps` 包)

---

## 📦 安装

```bash
go get your-module-path/hmap
```

---

## 🚀 快速开始

```go
package main

import "your-module-path/hmap"

func main() {
    // 创建 Map（默认 32 分片）
    m := hmap.New[string]()
    
    // 基本操作
    m.Set("name", "Alice")
    value, ok := m.Get("name")
    m.Del("name")
}
```

---

## 📚 API 文档

### 创建

```go
func New[V any](shardCount ...int) *Map[V]
```

| 参数 | 类型 | 说明 |
|-----|------|------|
| `shardCount` | `int` (可选) | 分片数量，自动向上取整为 2 的幂，默认 32 |

**示例：**
```go
m1 := hmap.New[int]()          // 默认 32 分片
m2 := hmap.New[string](64)     // 64 分片
m3 := hmap.New[*User](128)     // 128 分片，存储指针类型
```

---

### 基本操作

#### Set - 设置键值对

```go
func (m *Map[V]) Set(key string, value V)
```

```go
m.Set("user:1001", User{Name: "Alice"})
```

---

#### Get - 获取值

```go
func (m *Map[V]) Get(key string, defaultValue ...V) (V, bool)
```

| 返回值 | 说明 |
|-------|------|
| `V` | 键对应的值，不存在时返回零值或默认值 |
| `bool` | 键是否存在 |

```go
// 基本用法
value, ok := m.Get("user:1001")

// 带默认值
value, ok := m.Get("user:1001", User{Name: "Unknown"})
```

---

#### Del - 删除键

```go
func (m *Map[V]) Del(key string)
```

```go
m.Del("user:1001")
```

---

### 遍历与查询

#### Len - 获取元素数量

```go
func (m *Map[V]) Len() int
```

> ⚠️ **注意**：并发环境下返回的是近似值

---

#### Range - 遍历所有元素

```go
func (m *Map[V]) Range(f func(key string, value V) bool)
```

返回 `false` 可提前终止遍历。

```go
m.Range(func(key string, value int) bool {
    fmt.Printf("%s: %d\n", key, value)
    return true  // 继续遍历
})
```

> ⚠️ **注意**：由于涉及内存拷贝，在大数据量场景下请谨慎使用。

---

#### GetAllMaps - 获取所有数据的副本

```go
func (m *Map[V]) GetAllMaps() map[string]V
```

> ⚠️ **注意**：返回的是浅拷贝；并发环境下可能非完全一致的快照
> 
> ⚠️ **性能警告**: 会进行全量数据拷贝。

---

#### GetMaps - 获取指定分片数据

```go
func (m *Map[V]) GetMaps(index int) map[string]V
```

```go
shard0Data := m.GetMaps(0)  // 获取第 0 个分片的副本
```

---

### 其他操作

#### Clear - 清空所有数据

```go
func (m *Map[V]) Clear()
```

---

#### ShardCount - 获取分片数量

```go
func (m *Map[V]) ShardCount() int
```

---

## 💡 使用示例

### 基本用法

```go
package main

import (
    "fmt"
    "hmap"
)

func main() {
    // 创建存储用户信息的 Map
    users := hmap.New[User]()
    
    // 添加用户
    users.Set("u1", User{Name: "Alice", Age: 25})
    users.Set("u2", User{Name: "Bob", Age: 30})
    
    // 获取用户
    if user, ok := users.Get("u1"); ok {
        fmt.Printf("Found: %s\n", user.Name)
    }
    
    // 遍历
    users.Range(func(key string, user User) bool {
        fmt.Printf("%s -> %s\n", key, user.Name)
        return true
    })
    
    // 统计
    fmt.Printf("Total users: %d\n", users.Len())
}

type User struct {
    Name string
    Age  int
}
```

### 并发场景

```go
func main() {
    m := hmap.New[int](64)  // 高并发建议增加分片数
    
    var wg sync.WaitGroup
    
    // 并发写入
    for i := 0; i < 1000; i++ {
        wg.Add(1)
        go func(n int) {
            defer wg.Done()
            m.Set(fmt.Sprintf("key:%d", n), n)
        }(i)
    }
    
    // 并发读取
    for i := 0; i < 1000; i++ {
        wg.Add(1)
        go func(n int) {
            defer wg.Done()
            m.Get(fmt.Sprintf("key:%d", n))
        }(i)
    }
    
    wg.Wait()
    fmt.Printf("Final count: %d\n", m.Len())
}
```

---

## ⚠️ 注意事项

| 事项 | 说明 |
|-----|------|
| **Key 类型** | 目前仅支持 string 类型。如果需要其他类型的 Key，建议在外部转为 string 或修改源码中的 hash 计算部分 |
| **浅拷贝** | `GetMaps`/`GetAllMaps` 返回浅拷贝，引用类型需注意，修改返回的引用类型会导致原数据修改 |
| **非原子快照** | `Len`/`GetAllMaps` 在并发下可能不精确 |
| **遍历顺序** | `Range` 遍历顺序不确定 |
| **分片数量** | 高并发场景建议适当增加分片数（如 64、128） |
| **性能** | Range 方法为了保证绝对的线程安全（避免死锁），采用了“读时复制”策略。如果存储了数百万个对象，调用 Range 会导致大量的内存分配。<br>在核心热点路径上，应尽量避免频繁调用 Range 或 GetAllMaps。 |

---

## 🎯 性能建议

```
分片数量选择参考：
┌──────────────┬───────────────┐
│ 并发 Goroutine │ 建议分片数     │
├──────────────┼───────────────┤
│ < 100        │ 32（默认）     │
│ 100 - 1000   │ 64            │
│ > 1000       │ 128 - 256     │
└──────────────┴───────────────┘
```

---

## 📄 License

MIT License